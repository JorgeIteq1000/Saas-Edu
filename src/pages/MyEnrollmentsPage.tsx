...
interface Enrollment {
  ...
  course: {
    id: string; // Garantir que o ID do curso está na interface
    name: string;
    code: string;
...
const MyEnrollmentsPage = () => {
  ...
  const fetchMyEnrollments = async () => {
    try {
      ...
      // 5. Garantir que o ID do curso está sendo buscado na query
      const { data, error } = await supabase
        .from('enrollments')
        .select(`
          *,
          course:courses(id, name, code, duration_months)
        `)
  ...
  // 6. Adicionar a função para abrir o modal
  const handleOpenDisciplines = (course: {id: string, name: string}) => {
  ...
  return (
    <>
      ...
      {/* 9. Renderizar o modal */}
      <CourseDisciplinesModal
        open={isModalOpen}
