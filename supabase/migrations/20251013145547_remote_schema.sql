...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- log: Definindo o role para service_role para bypassar o RLS durante a inserção do perfil.
  ...
  -- log: Revertendo para o role original.
  reset role;
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION handle_new_user();
  ...
  create policy "Acesso Público de Leitura evj5ba_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using (true);
  ...
  create policy "Upload para Usuários Autenticados evj5ba_0"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check ((bucket_id = 'occurrence_images'::text));
