...
// Funções de criptografia e formatação (permanecem as mesmas)
async function getAccessToken(clientEmail: string, privateKey: string) {
  const jwtHeader = { alg: 'RS256', typ: 'JWT' };
  const now = Math.floor(Date.now() / 1000);
  const jwtPayload = { iss: clientEmail, scope: 'https://www.googleapis.com/auth/drive', aud: 'https://oauth2.googleapis.com/token', exp: now + 3600, iat: now };
  ...
  const tokenResponse = await fetch('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'urn:ietf:params:oauth:grant-type:jwt-bearer', assertion: signedJwt }) });
...
function base64url(data: string | Uint8Array) {
  const str = typeof data === 'string' ? data : String.fromCharCode.apply(null, data as unknown as number[]);
  return btoa(str).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}

// Função para concatenar arrays de bytes
...
serve(async (req) => {
  ...
  try {
    ...
    // --- AQUI ESTÁ A CORREÇÃO PARA O ARQUIVO CORROMPIDO ---
    const metadata = { name: fileName, parents: [FOLDER_ID] };
    const boundary = '----Boundary' + Math.random().toString(36).substring(2);
    const encoder = new TextEncoder();

    // Decodifica o base64 para um array de bytes (Uint8Array)
    ...
    // Constrói o corpo da requisição como um array de bytes, não como string
    const bodyParts = [
        ...
        fileBytes, // Usa o array de bytes diretamente
        encoder.encode(`\r\n--${boundary}--`),
    ];
    ...
    const uploadResponse = await fetch(uploadUrl, {
        ...
        body: body // Envia o corpo como dados binários
    });
    ...
    await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/permissions?supportsAllDrives=true`, { method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ role: 'reader', type: 'anyone' }) });
    const fileDetails = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=webViewLink&supportsAllDrives=true`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
